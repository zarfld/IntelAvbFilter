/**
 * @file avb_test_i226_um.c
 * @brief I226-specific user-mode validation test
 *
 * Enumerates all Intel adapters, locates an I226 (Foxville successor 2.5GbE),
 * opens it via IOCTL_AVB_OPEN_ADAPTER, and runs I226-specific hardware checks.
 * Prints [SKIP] and exits 0 if no I226 is present — safe for CI.
 *
 * I226-specific notes (verified against Intel I226 datasheet via NotebookLM):
 *   - 2.5 GbE (Foxville successor): 2500/1000/100/10 Mb/s
 ...
 *   - EEE IEEE 802.3az — key I226 differentiator over I225 (INTEL_CAP_EEE)
 *   - 20%-50% lower power than I225; ultra-low 18 mW with cable disconnected
 *
 ...
 *
 * NOTE: Does NOT modify or conflict with the existing nmake-based avb_i226_test.c / .mak
 *
...
/* Expected capability mask per Intel I226 datasheet */
#define I226_EXPECTED_CAPS  (INTEL_CAP_BASIC_1588 | INTEL_CAP_ENHANCED_TS | \
                             INTEL_CAP_TSN_TAS | INTEL_CAP_TSN_FP |         \
                             INTEL_CAP_PCIE_PTM | INTEL_CAP_2_5G |          \
                             INTEL_CAP_MMIO | INTEL_CAP_MDIO | INTEL_CAP_EEE)

/* I226 PCI Device IDs — SSOT: include/intel_pci_ids.h */
static int is_i226(avb_u16 did)
{
    return (did == INTEL_DEV_I226_LM ||   /* I226-LM 0x125B */
            did == INTEL_DEV_I226_V  ||   /* I226-V  0x125C */
            did == INTEL_DEV_I226_IT);    /* I226-IT 0x125D */
}

int main(void)
{
    printf("=== Intel I226 User-Mode Test ===\n\n");
    printf("Expected capabilities: BASIC_1588 | ENHANCED_TS | TSN_TAS | TSN_FP |"
           " PCIE_PTM | 2_5G | MMIO | MDIO | EEE\n");
    printf("(per Intel I226 datasheet, NotebookLM-verified — EEE is key I226 differentiator vs I225)\n\n");
    ...
    /* Find an I226 */
    const AvbAdapterInfo *i226 = NULL;
    for (int i = 0; i < count; i++) {
        if (is_i226(adapters[i].device_id)) {
            i226 = &adapters[i];
            break;
        }
    }
    ...
    if (!i226) {
        printf("[SKIP] No I226 adapter found among %d detected adapter(s).\n", count);
        for (int i = 0; i < count; i++) {
            char caps[128];
    ...
    {
        char caps[128];
        printf("[INFO] Found I226: DID=0x%04X global_index=%u caps=%s\n",
               i226->device_id, i226->global_index,
               AvbCapabilityString(i226->capabilities, caps, sizeof(caps)));
    }

    /* Open a handle bound to the I226 */
    HANDLE h = AvbOpenAdapter(i226);
    if (h == INVALID_HANDLE_VALUE) {
        printf("[FAIL] AvbOpenAdapter failed for I226 (error %lu)\n", GetLastError());
        return 1;
    }
    ...
    printf("\n--- Test 3: PTP Hardware Timestamping Enable ---\n");
    {
        ...
        hwts.enable             = 1;
        hwts.timer_mask         = 1;
        hwts.enable_target_time = 1;   /* I226 supports TRGTTIM */
        hwts.enable_aux_ts      = 1;   /* I226 supports AUXSTMP */
     ...
     *
     * Per Intel I226 datasheet (NotebookLM-verified):
     *   MUST have: BASIC_1588, ENHANCED_TS, TSN_TAS, TSN_FP, PCIE_PTM, 2_5G, MMIO, MDIO, EEE
     *   EEE is the key differentiator vs I225 (which lacks EEE).
     */
    printf("\n--- Test 5: Capabilities Verification (per Intel I226 datasheet) ---\n");
    {
        char capbuf[128];
        avb_u32 caps = i226->capabilities;
        printf("  [INFO] Capabilities = 0x%08X (%s)\n",
               (unsigned)caps, AvbCapabilityString(caps, capbuf, sizeof(capbuf)));
        printf("  [INFO] Expected     = 0x%08X\n", (unsigned)I226_EXPECTED_CAPS);
        ...
        if (caps & INTEL_CAP_TSN_TAS) {
            ...
            AVB_REPORT_FAIL(&stats, "INTEL_CAP_TSN_TAS",
                            "TAS not reported — I226 datasheet confirms 802.1Qbv support");
        }
        ...
        if (caps & INTEL_CAP_TSN_FP) {
            ...
            AVB_REPORT_FAIL(&stats, "INTEL_CAP_TSN_FP",
                            "Frame Preemption not reported — I226 datasheet confirms 802.1Qbu/802.3br");
        }
        ...
        if (caps & INTEL_CAP_PCIE_PTM) {
            ...
            AVB_REPORT_FAIL(&stats, "INTEL_CAP_PCIE_PTM",
                            "PCIe PTM not reported — I226 datasheet confirms PTM support");
        }
        ...
        if (caps & INTEL_CAP_2_5G) {
            ...
            AVB_REPORT_FAIL(&stats, "INTEL_CAP_2_5G",
                            "2.5G not reported — I226 supports 2500/1000/100/10 Mb/s");
        }

        if (caps & INTEL_CAP_MMIO) {
            AVB_REPORT_PASS(&stats, "INTEL_CAP_MMIO present (BAR0 register access)");
        } else {
            AVB_REPORT_FAIL(&stats, "INTEL_CAP_MMIO", "MMIO not reported");
        }
        ...
        if (caps & INTEL_CAP_MDIO) {
            ...
            AVB_REPORT_FAIL(&stats, "INTEL_CAP_MDIO",
                            "MDIO not reported — I226 datasheet confirms MDC/MDIO interface");
        }

        /* EEE is the key I226 differentiator — must be present */
        if (caps & INTEL_CAP_EEE) {
            AVB_REPORT_PASS(&stats, "INTEL_CAP_EEE present (IEEE 802.3az — I226 key differentiator vs I225)");
        } else {
            AVB_REPORT_FAIL(&stats, "INTEL_CAP_EEE",
                            "EEE not reported — I226 datasheet confirms 802.3az EEE (exclusive vs I225)");
        }
    }
    ...
    /* ── Test 7: I226 Variant Matrix ── */
    printf("\n--- Test 7: I226 Variant Matrix ---\n");
    {
        static const struct { avb_u16 did; const char *name; } kI226Variants[] = {
            { INTEL_DEV_I226_LM, "I226-LM  2.5G (0x125B)" },
            { INTEL_DEV_I226_V,  "I226-V   2.5G (0x125C)" },
            { INTEL_DEV_I226_IT, "I226-IT  2.5G (0x125D)" },
        };
        static const int kI226VariantCount =
            (int)(sizeof(kI226Variants) / sizeof(kI226Variants[0]));

        printf("  [INFO] Full I226 variant table (%d entries):\n", kI226VariantCount);
        const char *matchName = NULL;
        for (int vi = 0; vi < kI226VariantCount; vi++) {
            int current = (kI226Variants[vi].did == i226->device_id) ? 1 : 0;
            printf("  [INFO]   DID=0x%04X  %-30s %s\n",
                   kI226Variants[vi].did,
                   kI226Variants[vi].name,
                   current ? "<-- THIS DEVICE" : "");
            if (current) { matchName = kI226Variants[vi].name; }
        }

        if (matchName) {
            printf("  [PASS] DID=0x%04X recognized as %s\n",
                   (unsigned)i226->device_id, matchName);
            stats.passed++; stats.total++;
        } else {
            printf("  [FAIL] DID=0x%04X not in recognized I226 variant table\n",
                   (unsigned)i226->device_id);
            printf("         Update kI226Variants[] in avb_test_i226_um.c\n");
            stats.failed++; stats.total++;
        }
    }
    ...
    printf("\n--- Test 8: TSN TAS Gate Open (SETUP_TAS accepted) ---\n");
    {
        ...
        if (!result && err == ERROR_NOT_SUPPORTED) {
            AVB_REPORT_FAIL(&stats, "SETUP_TAS",
                            "Returned ERROR_NOT_SUPPORTED on I226 — capability gate incorrectly blocking");
        } else {
    ...
    printf("\n--- Test 9: TSN FP Gate Open (SETUP_FP accepted) ---\n");
    {
        ...
        fpReq.config.preemptable_queues = 0x0FU;
    ...
    printf("\n--- Test 11: MDIO PHY Register Read ---\n");
    {
        ...
        /* Read PHY Status Register to confirm OUI non-zero */
        ZeroMemory(&mdioReq, sizeof(mdioReq));
        mdioReq.page = 0;
        mdioReq.reg  = 1;
        result = DeviceIoControl(h, IOCTL_AVB_MDIO_READ,
        ...
        if (!result || mdioReq.status != 0) {
            ...
            AVB_REPORT_FAIL(&stats, "MDIO PHY reg 1", "Value=0xFFFF");
        } else {
