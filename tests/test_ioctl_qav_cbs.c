...
// IOCTL Definitions
#define IOCTL_AVB_CONFIGURE_CBS CTL_CODE(FILE_DEVICE_NETWORK, 0x835, METHOD_BUFFERED, FILE_ANY_ACCESS)
...
#pragma pack(push, 1)
typedef struct _CBS_CONFIG_REQUEST {
    UCHAR  traffic_class;     // 0 = Class A, 1 = Class B
    UINT32 idle_slope;        // Credit increase rate (bps or percentage)
    INT32  send_slope;        // Credit decrease rate (negative value)
    UINT32 hi_credit;         // Maximum positive credit (bytes)
    INT32  lo_credit;         // Maximum negative credit (bytes, negative value)
