...
// If this file is compiled inside the driver project, include PCH to satisfy settings
#if defined(_MSC_VER)
...
public:
    ...
    TEST_METHOD(Gptp_SetThenGet)
    {
        ...
        if (!setOk) {
            Log::Comment(L"SET_TIMESTAMP unsupported by hardware/driver; skipping");
            WEX::TestExecution::SetVerifyOutput verifySettings(VerifyOutputSettings::LogOnlyFailures);
            return; // soft-skip
        }
    ...
    TEST_METHOD(Timestamp_Monotonicity_10k)
    {
        const size_t N = 10000;
        std::vector<uint64_t> v; v.reserve(N);
        AVB_TIMESTAMP_REQUEST req = {}; DWORD bytes = 0;
        ...
        for (size_t i = 1; i < v.size(); ++i) { if (v[i] < v[i-1]) { mono = false; break; } maxStep = std::max<uint64_t>(maxStep, v[i]-v[i-1]); }
        VERIFY_IS_TRUE(mono, L"Timestamp must be non-decreasing");
        Log::CommentFmt(L"Max step = %llu ns", (unsigned long long)maxStep);
    }

    TEST_METHOD(Throughput_1s_GetTimestamp)
    {
        uint64_t end = qpc_ns() + 1000000000ULL; // 1s
        AVB_TIMESTAMP_REQUEST req = {}; DWORD bytes = 0; uint64_t cnt = 0;
        while (qpc_ns() < end) { if (!IoctlInOut(IOCTL_AVB_GET_TIMESTAMP, &req, sizeof(req), &req, sizeof(req), bytes)) break; ++cnt; }
        Log::CommentFmt(L"GET_TIMESTAMP throughput: %.2f kops/s", (double)cnt / 1000.0);
        VERIFY_IS_TRUE(cnt > 1000, L"Expect at least minimal throughput");
    }

    TEST_METHOD(ErrorPath_DisableEnable_Interface)
    {
        wchar_t name[256] = {0};
        DWORD got = GetEnvironmentVariableW(L"AVB_ADAPTER_NAME", name, _countof(name));
        if (got == 0 || got >= _countof(name)) {
            Log::Comment(L"Set AVB_ADAPTER_NAME to interface friendly name to run this test");
            WEX::TestExecution::SetVerifyOutput verifySettings(VerifyOutputSettings::LogOnlyFailures);
            return; // soft-skip
        }
        wchar_t cmd[512]; wsprintfW(cmd, L"netsh interface set interface \"%ls\" admin=disabled", name);
        if (_wsystem(cmd) != 0) {
            Log::Comment(L"netsh failed; skipping");
            WEX::TestExecution::SetVerifyOutput verifySettings(VerifyOutputSettings::LogOnlyFailures);
            return;
        }
        ::Sleep(2000);
        ...
        _wsystem(cmd);
        ::Sleep(3000);
        bool ok = false; for (int i = 0; i < 10; ++i) { if (IoctlInOut(IOCTL_AVB_GET_TIMESTAMP, &req, sizeof(req), &req, sizeof(req), bytes)) { ok = true; break; } ::Sleep(500); }
