...
BOOL test_initialization_sequence(void)
{
    ...
    printf("? Initialization IOCTL succeeded\n");
...
BOOL test_multi_adapter_states(void)
{
    ...
    for (UINT32 i = 0; i < g_ctx.adapter_count && i < 3; i++) {  // Test up to 3 adapters
        ...
        DWORD bytesReturned;
        BOOL enum_ok = DeviceIoControl(g_ctx.device, IOCTL_AVB_ENUM_ADAPTERS,
                                      &enum_req, sizeof(enum_req), &enum_req, sizeof(enum_req),
                                      &bytesReturned, NULL);

        if (!enum_ok) {
            printf("    ? Adapter %u: Enumeration failed (%lu)\n", i, GetLastError());
            continue;
        }
    ...
    if (result && reg_req.status != 0) {
        ...
        if (hw_state_get_current(&current_state, NULL, NULL, NULL)) {
            ...
            if (current_state == 3) {  // ERROR state
                ...
                (void)DeviceIoControl(g_ctx.device, IOCTL_AVB_INIT_DEVICE,
                                     NULL, 0, NULL, 0,
                                     &bytesReturned, NULL);
