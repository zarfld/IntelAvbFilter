...
/*
 * Files excluded from scanning — auto-generated or third-party files where
 * raw hex literals are expected and should not be abstracted.
 *
 * IntelAvbFilter.h is generated by the Windows Message Compiler (MC.exe) from
 * IntelAvbFilter.man.  Its hex literals are ETW GUID components
 * (0x5dc1ca46...), platform version guards (0x06040000 = NTDDI_WIN10), and
 * ETW keyword bitmasks (0x8000000000000000) — all values owned by the ETW
 * framework and not replaceable with SSOT driver constants.
 */
static const char * const EXCLUDED_FILES[] = {
    "IntelAvbFilter.h"   /* MC.exe-generated ETW manifest header */
};
static const int EXCLUDED_FILE_COUNT = 1;
...
static void ScanFile(const char *path)
{
    ...
    /* Skip excluded files (matched by filename only, not full path) */
    basename = strrchr(path, '\\');
    basename = basename ? basename + 1 : path;
    for (i = 0; i < EXCLUDED_FILE_COUNT; i++) {
        if (_stricmp(basename, EXCLUDED_FILES[i]) == 0)
            return;
    }
