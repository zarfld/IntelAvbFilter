...
| ~~[#204](https://github.com/zarfld/IntelAvbFilter/issues/204)~~ | ~~Target Time Interrupt: UT-TARGET-004 (past-time detection) not covered by any test~~ | ✅ **DONE 2026-03-29** — TC-TARGET-005/006/007 all PASS; **15P/0F/1S** — driver guard `2ad239e` in `IOCTL_AVB_SET_TARGET_TIME` rejects `target_time < current_systim` with `STATUS_INVALID_PARAMETER` (Win32 error 87). Log: `test_ioctl_target_time.exe_20260329_*.log`. |
| [#209](https://github.com/zarfld/IntelAvbFilter/issues/209) | Launch Time Offload — `IOCTL_AVB_SET_LAUNCH_TIME` / `TQAVLAUNCHTIME` for per-packet TX scheduling (IEEE 802.1Qbv): UT-LAUNCH-001..010, IT-LAUNCH-001..003, VV-LAUNCH-001..002 — no test or driver API exists | ⚠️ **RE-OPENED 2026-03-30** — Attribution error: close on 2026-03-29 was wrong; `2ad239e` + TC-TARGET-005 belong to **#204**. This is a separate, unimplemented feature. Backlog. |
...
| ~~[#288](https://github.com/zarfld/IntelAvbFilter/issues/288)~~ | ~~PCI read latency <100 µs assertion~~ | ✅ **DONE 2026-03-29** — TC-PCI-LAT-001/002/003 added to `test_hw_state.c`; **12/12 PASS** (P99<250µs, outlier<15ms; thresholds empirically justified from 5 runs w/ Release + Debug drivers) |
