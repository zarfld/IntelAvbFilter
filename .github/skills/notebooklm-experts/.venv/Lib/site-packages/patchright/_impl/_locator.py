...
from patchright._impl._js_handle import Serializable
...
class Locator:
    ...
    async def is_hidden(self, timeout: float = None) -> bool:
        return await self._frame.is_hidden(self._selector, strict=True)

    async def is_visible(self, timeout: float = None) -> bool:
        return await self._frame.is_visible(self._selector, strict=True)
    ...
    async def _expect(
        self, expression: str, options: FrameExpectOptions, title: str = None
    ) -> FrameExpectResult:
