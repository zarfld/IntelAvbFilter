...
static int init_ptp(device_t *dev)
{
    ...
    uint32_t timinca = INTEL_TIMINCA_I350_INIT;  // 8ns increment for I350
...
/**
 * @brief Enable/disable packet timestamping (I350-specific)
 * @param dev Device handle
 * @param enable 1 = enable, 0 = disable
 * @return 0 on success, <0 on error
 * 
 * Configures TSYNCRXCTL and TSYNCTXCTL registers for I350 (IGB family).
 */
static int enable_packet_timestamping(device_t *dev, int enable)
{
    // I350 PTP registers (IGB family)
    ...
    if (enable) {
        ...
        if (ndis_platform_ops.mmio_write(dev, TSYNCRXCTL, rx_ctl) != 0) {
            DEBUGP(DL_ERROR, "I350: Failed to write TSYNCRXCTL\n");
            return -1;
        }
        DEBUGP(DL_INFO, "I350: Enabled RX packet timestamping (TSYNCRXCTL=0x%08X)\n", rx_ctl);
        
        // Enable TX packet timestamping
        uint32_t tx_ctl = TXTT_ENABLE;
        if (ndis_platform_ops.mmio_write(dev, TSYNCTXCTL, tx_ctl) != 0) {
            DEBUGP(DL_ERROR, "I350: Failed to write TSYNCTXCTL\n");
            return -1;
        }
        DEBUGP(DL_INFO, "I350: Enabled TX packet timestamping (TSYNCTXCTL=0x%08X)\n", tx_ctl);
    } else {
        ...
        DEBUGP(DL_INFO, "I350: Disabled packet timestamping\n");
    }
